...
mod <- ghype(adj,directed = F, selfloops = F, unbiased = T)

blockmatrix <- V(karate)$color%*%t(V(karate)$color)
blockmatrix[blockmatrix==4] <- 1
blockmatrix[blockmatrix==2] <- .1
blockModel <- ghype(adj,directed = F, selfloops = F, omega = blockmatrix)
blockModel2 <- bccm(adj = adj, labels = V(karate)$Faction, directed = F, selfloops = F, homophily = T)
```
